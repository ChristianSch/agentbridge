...
async function status(){try{const s=await api('/auth/status'); if(s.authenticated){$('status').textContent='Unlocked.'; $('enter').style.display='inline-block'} else {$('status').textContent=s.registered?'Passkey required.':'No passkey registered yet. Register with your bootstrap token.'}}catch(e){$('status').textContent='Locked.'}}
