...
function App() {
  ...
  function addMessage(sessionId, msg) {
    setMessages(prev => {
      ...
      if (msg.type === 'activity_summary') {
        const lastUser = list.map(x => x.type).lastIndexOf('user')
        let existing = -1
        for (let i = list.length - 1; i > lastUser; i--) if (list[i].type === 'activity_summary') { existing = i; break }
        if (existing >= 0) return { ...prev, [sessionId]: [...list.slice(0, existing), msg, ...list.slice(existing + 1)] }
      }
  ...
  return <div class={`app ${sidebarOpen ? 'sidebar-open' : ''}`}>
    <button class="mobile-menu" type="button" aria-label="Switch conversation" aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(true)}>☰</button>
    {sidebarOpen && <button class="drawer-scrim" type="button" aria-label="Close conversations" onClick={() => setSidebarOpen(false)} />}
    <aside class="sidebar"><div class="brand"><span>AgentBridge</span> <small>{socketState}</small><button class="drawer-close" type="button" aria-label="Close conversations" onClick={() => setSidebarOpen(false)}>×</button></div><div class="create"><button class="new-session" onClick={() => { setFlowKind('pi'); setSidebarOpen(false) }}>＋ New session</button></div>{detectInfo && !detectInfo.hermes_profiles?.length && <div class="notice">Hermes not detected</div>}<div class="sessions">{sessions.map(s => <SessionItem session={s} active={s.id === active} onSelect={() => selectSession(s.id)} onRename={renameSession} onDelete={deleteSession} />)}</div></aside>
    <main class="pane">{!activeSession && <Empty onNew={setFlowKind} />}{activeSession?.kind === 'terminal' && <TerminalPane session={activeSession} />}{activeSession && activeSession.kind !== 'terminal' && <AgentChat session={activeSession} messages={messages[active] || []} onSend={(action, text, extra) => send(action, text, extra, activeSession.id)} />}</main>
...
function NewSessionFlow({ initialKind, hermesAvailable, onCancel, onCreate }) {
  ...
  return <div class="modal-backdrop" onClick={onCancel}><form class="modal" onClick={e => e.stopPropagation()} onSubmit={submit}>
    ...
    <div class="kind-picker"><button type="button" class={kind === 'pi' ? 'selected' : ''} onClick={() => setKind('pi')}>Pi<small>coding agent</small></button><button type="button" disabled={!hermesAvailable} class={kind === 'hermes' ? 'selected' : ''} onClick={() => setKind('hermes')}>Hermes<small>{hermesAvailable ? 'local gateway' : 'not detected'}</small></button><button type="button" class={kind === 'terminal' ? 'selected' : ''} onClick={() => setKind('terminal')}>Term<small>use-once shell</small></button></div>
...
function AgentChat({ session, messages, onSend }) {
  const [text, setText] = useState(''), [visibleCount, setVisibleCount] = useState(300), [showCommands, setShowCommands] = useState(false), [toolsOpen, setToolsOpen] = useState(false), [attachments, setAttachments] = useState([]), [uploading, setUploading] = useState(false), [recording, setRecording] = useState(false), [voiceError, setVoiceError] = useState(''), bottom = useRef(null), fileInput = useRef(null), textInput = useRef(null), recorder = useRef(null), chunks = useRef([])
  useEffect(() => { setText(''); setVisibleCount(300); setShowCommands(false); setToolsOpen(false); setAttachments([]); setVoiceError('') }, [session.id])
...
function TerminalPane({ session }) { const ref = useRef(null); useEffect(() => { const term = new Terminal({ fontSize: 14, cursorBlink: true, theme: { background: '#15171d' } }); const fit = new FitAddon(); term.loadAddon(fit); term.open(ref.current); fit.fit(); const ws = new WebSocket(`${location.protocol === 'https:' ? 'wss' : 'ws'}://${location.host}/ws/term/${session.id}`); ws.binaryType = 'arraybuffer'; ws.onmessage = e => { if (typeof e.data !== 'string') term.write(new Uint8Array(e.data)) }; ws.onopen = () => resize(term, ws, fit); term.onData(data => ws.readyState === WebSocket.OPEN && ws.send(data)); const onResize = () => resize(term, ws, fit); window.addEventListener('resize', onResize); const timer = setTimeout(onResize, 50); return () => { clearTimeout(timer); window.removeEventListener('resize', onResize); ws.close(); term.dispose() } }, [session.id]); return <><header class="top"><div><strong>{session.name}</strong><span>terminal · use-once shell · not restored after restart</span></div><code>{session.id}</code></header><div class="terminal" ref={ref} /></> }
